...
class BsdGiuChoThienChi(models.Model):
    ...
    def _compute_chuyen_ut_gc(self):
        for each in self:
            chuyen_ut_gc = self.env['bsd.chuyen_ut_gc'].search([('bsd_gc_tc_ch_id', '=', self.id)])
    ...
    def action_view_chuyen_ut_gc(self):
        ...
        chuyen_gc = self.env['bsd.chuyen_ut_gc'].search([('bsd_gc_tc_ch_id', '=', self.id)])
        if len(chuyen_gc) > 1:
    ...
    def action_chuyen_ut_gc(self):
        context = {
            'default_bsd_gc_tc_ch_id': self.id,
            'default_bsd_du_an_id': self.bsd_du_an_id.id,
            'default_bsd_loai_gc': 'gc_tc',
        }
