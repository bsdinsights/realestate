...
class BsdLaiSuat(models.Model):
    ...
    bsd_ma = fields.Char(string="Mã", help="Mã lãi suất", required=True)
    _sql_constraints = [
        ('bsd_ma_unique', 'unique (bsd_ma)',
         'Mã lãi suất đã tồn tại !'),
    ]
    bsd_ten = fields.Char(string="Tên", help="Tên lãi suất", required=True)
    bsd_du_an_id = fields.Many2one('bsd.du_an', string="Dự án", required=True, help="Tên dự án")
    bsd_ngay_ap_dung = fields.Date(string="Ngày", required=True, help="Ngày áp dụng")
